...
const Buy = () => {
  ...
  const handleCard = (item: CardAsset) => {
    ...
    setSelectedCurrency(null); // Reset currency on new card selection
    setSelectedType(null);
    setSelectedValue(null);
    setUnit(1);
  };
    ...
    <div className="w-full">
      <form className="w-full">
        ...
        <div className="w-full flex lg:flex-row sm:flex-col items-center gap-3">
          <div className={`transition-all duration-300 ${selectedValue ? "lg:w-[70%]" : "w-full"} sm:w-full`}>
            ...
            <Select
              ...
              disabled={!selectedCard} // Enabled when a gift card is selected
            />
          </div>
          {selectedValue && (
            <div className="lg:w-[30%] sm:w-full">
              ...
              <div className="w-full border-2 rounded-lg flex border-primary-1">
                <button
                  ...
                  className="border-r-2 py-3.5 w-[30%] border-primary-1 lg:text-base sm:text-md"
                >
                  -
                </button>
                <input
                  type="number"
                  value={unit}
                  className="text-center bg-transparent outline-none py-3.5 w-[30%] lg:text-base sm:text-md"
                  readOnly
                />
                <button
                  type="button"
                  onClick={() => handleAmountChange("increase")}
                  className="border-l-2 py-3.5 w-[30%] border-primary-1 lg:text-base sm:text-md"
                >
                  +
                </button>
              </div>
            ...
            <div className="w-full grid lg:grid-cols-4 sm:grid-cols-2 gap-3 text-secondary-1">
              {giftCardValues.map((value) => (
                <button
                  key={value}
                  type="button"
                  className={`lg:p-3 sm:p-2 border-2 rounded-xl text-center cursor-pointer ${
                    selectedValue === value ? "bg-primary-1 text-white" : "border-primary-1"
                  }`}
        ...
        {selectedCrypto && selectedCurrency && selectedValue !== null && (
          <div className="lg:p-[15px] mt-3 bg-primary-1 rounded-2xl flex flex-col sm:p-3">
            <div className="flex justify-between">
              ...
              <h2 className="flex items-center gap-x-2">
                ...
                <span>/</span>
                <span className="uppercase text-secondary-1 font-bold lg:text-base sm:text-md">{selectedCurrency.title}</span>
              </h2>
            </div>
